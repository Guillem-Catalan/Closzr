...
/* =========================================================
   MAIN VIEW
   ========================================================= */
export default function OneOnOneView(_props: { onOpen: (row: any, tab?: string) => void }) {
  ...
  const doAction = useCallback((dk: string, deal: OODeal, mode: ActionMode, secNum: string) => {
    ...
    if (mode === "keep") {
      ...
      if (deal.hs_deal_id) {
        callDealUpdate("add_note", { hs_deal_id: deal.hs_deal_id, note: `[1:1] Fecha mantenida: ${reason}` });
      }
    } else if (mode === "change") {
      ...
      if (deal.hs_deal_id) {
        callDealUpdate("update_close_date", { hs_deal_id: deal.hs_deal_id, new_date: newDate });
        callDealUpdate("add_note", { hs_deal_id: deal.hs_deal_id, note: `[1:1] Fecha cambiada: ${reason}` });
      }
    } else if (mode === "action" || mode === "push") {
      addEntry({ deal_id: deal.deal_id, deal_name: name, section: secNum, type: "commitment", note: reason });
      if (deal.hs_deal_id) {
        callDealUpdate("add_note", { hs_deal_id: deal.hs_deal_id, note: `[1:1] ${mode === "push" ? "Adelantar" : "Acción"}: ${reason}` });
      }
    } else if (mode === "lost") {
      addEntry({ deal_id: deal.deal_id, deal_name: name, section: secNum, type: "change",
      ...
      if (deal.hs_deal_id) {
        callDealUpdate("mark_lost", { hs_deal_id: deal.hs_deal_id, lost_reason: lostReason, detail: reason });
      }
    }
  ...
  const changeFc = useCallback((dealId: string, dealName: string, from: string, to: string) => {
    ...
    if (deal?.hs_deal_id) {
      callDealUpdate("update_forecast", { hs_deal_id: deal.hs_deal_id, category: to });
    }
    week.sections.forEach(s => s.items.forEach(i => {
      if (itemDeals(i).some(d => d.deal_id === dealId)) autoCheck(i.id);
    }));
  }, [addEntry, deals, callDealUpdate, week, itemDeals, autoCheck]);
              ...
              <>
                ...
                {guideOpen && (
                  <div className="guide">
                    {item.guide.map((g, i) => {
                      const sep = g.indexOf("|");
                      const tag = sep > 0 ? g.slice(0, sep) : null;
                      const txt = sep > 0 ? g.slice(sep + 1) : g;
                      return (
                        <div key={i} className="guide-i">
                          {tag ? <span className="guide-tag">{tag}</span> : <span className="guide-b" />}
                          <span>{txt}</span>
                        </div>
                      );
                    })}
