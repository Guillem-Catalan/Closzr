...
export function useOneOnOne(repName: string, weekType: number, tlEmail: string, monday: string, profile: UserProfile | null) {
  ...
  useEffect(() => {
    ...
    supabase.from("org_people").select("full_name,email")
      .then(({ data }) => {
        const map = new Map<string, string>();
        for (const r of data || []) {
          if (r.full_name && r.email) map.set(r.full_name, r.email.toLowerCase());
        }
        setNameToEmail(map);
      });
  }, []);
  ...
  const resolveEmail = useCallback(
    (name: string) => nameToEmail.get(name) || repNameToEmail(name),
    [nameToEmail],
  );
  ...
  const reps = useMemo(() => {
    ...
    if (scope === "self") {
      ...
      return allReps.filter(n => resolveEmail(n) === own);
    }
    const subtree = new Set(profile.subtreeEmails.map(e => e.toLowerCase()));
    return allReps.filter(n => subtree.has(resolveEmail(n)));
  }, [allReps, profile, resolveEmail]);
  ...
  useEffect(() => {
    ...
    const email = resolveEmail(repName);
    const m = monthKey(0);
